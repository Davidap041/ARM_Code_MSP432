...
Control_Law_data controller_Rho = { .identification = 0,
                                    .PI_Kp = 50.0f, //10.0f
        .PI_Ki = 0.1f, .upper_limit = 3.6f, .lower_limit = -3.6f };
// upper = 5.0f and lower  -0.5f for sin wave
...
int8_t DR_centralize_Engine(uint32_t stabilization_time, float referencia)
{
    ...
    {
        Dr_set_led;
        float Control_Signal = PI_Control_Law(&controller_Rho, referencia);
...
void DR_acquisition_data(uint16_t n_ensaio)
{
    ...
    if (n_ensaio == 2)
    {
        ...
        printf("\n\r%d %d %d %d %d", time_acquisition, aq_data2[0], aq_data2[1],
...
//------------------- Routines outside of Data acquisition ----------------------//
    ...
    { // Engine calibration and centralization routines
        ...
        { // centralize the engine
            int8_t stabilizing = 0;
            if (inicializar_square)
            {
                stabilizing = DR_centralize_Engine(500, -0.5);
            }
            else
            {

                stabilizing = DR_centralize_Engine(500, 0);
            }
